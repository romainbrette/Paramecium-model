'''
Selection of deciliated cells (for Fig. 3).
...
table_folder = join(config['root'],'Deciliated', 'tables')

# Make the panda table
df = pd.read_csv(join(table_folder,'deciliated_bell_p2.csv'))
...
df_selected.to_csv(join(table_folder,'selection_deciliated.csv'))
df_selected.to_excel(join(table_folder,'selection_deciliated.xlsx'))
