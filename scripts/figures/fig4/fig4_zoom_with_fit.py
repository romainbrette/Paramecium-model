...
#description['model']['constants']['gCa_cilia'] = 0*amp # to see what happens if the calcium current disappears
...
ax_V.plot(t / ms, v[i]/mV, linestyle='--', color=color)
ax_current.plot(t[:-1] / ms, filtered, color=color, alpha=1)
#Ifit = C * diff(v[i])/dt - Ii[:-1] - gL*(EL-v[i,:-1])
#ax_current.plot(t[:-1] / ms, -Ifit/nA, linestyle='--', color=color, alpha=1)
